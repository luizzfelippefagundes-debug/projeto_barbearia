...
export function NextClientCallout() {
  ...
  const proximo = state.filaEspera.find((f) => !f.notificado)

  const cliente = proximo ? state.clientes.find((c) => c.id === proximo.clienteId) : undefined
  const barbeiroDesejado = proximo?.desejaBarbeiroId
    ? state.barbeiros.find((b) => b.id === proximo.desejaBarbeiroId)
    : undefined
