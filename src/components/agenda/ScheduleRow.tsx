...
export function ScheduleRow({ agendamento, barbeiros, clientes, servicos }: ScheduleRowProps) {
  const barbeiro = barbeiros.find((b) => b.id === agendamento.barbeiroId)
  const cliente = agendamento.clienteId
    ? clientes.find((c) => c.id === agendamento.clienteId)
    : undefined
  const servico = agendamento.servicoId
    ? servicos.find((s) => s.id === agendamento.servicoId)
    : undefined
